  ...
  return (
    <>
      ...
      <div 
        ...
        className="bg-white border border-slate-200 rounded-xl md:rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-green-500/40 transition-all duration-200 cursor-pointer group flex flex-col sm:flex-row"
      >
        <div className="bg-slate-900 px-4 md:px-6 py-3 md:py-4 flex flex-row sm:flex-col justify-between sm:justify-center items-center sm:items-start sm:w-48 shrink-0">
          <span className="px-2.5 py-1 bg-green-500/20 text-green-400 text-[9px] md:text-[10px] font-bold rounded-md uppercase tracking-widest border border-green-500/30 shadow-sm sm:mb-3">
            {supply.status.replace("_", " ")}
          </span>
          <div className="flex items-center gap-1.5 md:gap-2 text-slate-400 text-[10px] md:text-xs font-medium">
            <Clock size={12} className="text-green-500 md:w-3.5 md:h-3.5" />
        ...
        <div className="p-4 md:p-5 flex-1 flex flex-col justify-center">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 md:gap-4 mb-2 md:mb-3">
            <div>
              <h3 className="text-base md:text-lg font-bold text-slate-900 tracking-tight group-hover:text-green-600 transition-colors leading-tight">{supply.title}</h3>
            ...
            <div className="text-left sm:text-right shrink-0 mt-1 sm:mt-0">
              ...
              {supply.price ? (
                <p className="text-xs md:text-sm font-bold text-green-600 mt-0.5">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(supply.price)}</p>
              ) : (
        ...
        <div className="hidden sm:flex bg-slate-50 px-4 items-center justify-center border-l border-slate-100 group-hover:bg-green-50/50 transition-colors">
          ...
          <div className="bg-white rounded-2xl md:rounded-3xl w-full max-w-4xl max-h-[95vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-4 md:px-6 py-3 md:py-4 border-b border-slate-100 flex justify-between items-center bg-slate-900 text-white shrink-0">
              <div className="flex items-center gap-3">
                <div className="bg-green-600 p-2 rounded-xl shadow-inner"><Package size={20} className="text-white" /></div>
                <h2 className="text-lg md:text-xl font-bold tracking-wide">Supply Details</h2>
              </div>
              ...
              <div className="flex items-center gap-1.5 md:gap-3">
                ...
                {canManage && (
                  <div className="flex items-center gap-1.5 md:gap-2 mr-1 pr-2 md:mr-3 md:pr-4 border-r border-slate-700">
                    {/* The SupplyForm handles the Edit logic. Wrapper ensures alignment. */}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 lg:p-8 bg-slate-50">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                ...
                <span className="px-3 py-1.5 bg-green-100 text-green-800 text-[10px] md:text-xs font-bold rounded-lg uppercase tracking-widest border border-green-200 shrink-0">
                  {supply.status.replace("_", " ")}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
                <div className="bg-white border border-slate-200 p-4 md:p-5 rounded-2xl flex flex-col shadow-sm">
                  <div className="flex items-center gap-2 mb-2 text-slate-500"><Scale size={16} className="text-green-600"/> <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider">Available Qty</span></div>
                  <p className="text-xl md:text-2xl font-black text-slate-900 truncate">{new Intl.NumberFormat().format(supply.quantity)} <span className="text-xs md:text-sm font-bold text-slate-500 ml-0.5">{supply.quantityUnit || "MT"}</span></p>
                </div>
                <div className="bg-white border border-slate-200 p-4 md:p-5 rounded-2xl flex flex-col shadow-sm">
                  ...
                  {supply.price ? (
                    <p className="text-xl md:text-2xl font-black text-slate-900 truncate">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(supply.price)}</p>
                  ) : (
                    <p className="text-sm md:text-base font-bold text-slate-400 italic mt-1">Price Upon Request</p>
                  )}
                </div>
                <div className="bg-white border border-slate-200 p-4 md:p-5 rounded-2xl flex flex-col shadow-sm">
                  <div className="flex items-center gap-2 mb-2 text-slate-500"><MapPin size={16} className="text-green-600"/> <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider">Location</span></div>
                  <p className="text-sm md:text-base font-bold text-slate-900 truncate mt-1" title={supply.location}>{supply.location}</p>
                </div>
                <div className="bg-white border border-slate-200 p-4 md:p-5 rounded-2xl flex flex-col shadow-sm">
                  <div className="flex items-center gap-2 mb-2 text-slate-500"><CalendarClock size={16} className="text-green-600"/> <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-wider">Offer Validity</span></div>
                  <p className="text-sm md:text-base font-bold text-rose-600 truncate mt-1">
              ...
              <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 mb-6 md:mb-8 shadow-sm">
                <h4 className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <Truck size={14} className="md:w-4 md:h-4 text-green-600" /> Trade Logistics
                </h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-3 md:gap-4">
                  ...
                  <LogisticsItem label="Load Port" value={supply.loadPort} icon={<Anchor size={14} className="text-slate-400" />} />
                  <LogisticsItem label="Incoterms" value={supply.incoterms} icon={<Truck size={14} className="text-slate-400" />} />
                  <LogisticsItem label="Insurance" value={supply.insurance} icon={<Shield size={14} className="text-slate-400" />} />
              ...
              {supply.keyTerms && Array.isArray(supply.keyTerms) && supply.keyTerms.length > 0 && (
                <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 mb-6 md:mb-8 shadow-sm">
                  <h4 className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                    <List size={14} className="md:w-4 md:h-4 text-green-600" /> Technical Specifications
                  </h4>
              ...
              <div className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200 mb-6 md:mb-8 shadow-sm">
                <h4 className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                  <FileText size={14} className="md:w-4 md:h-4 text-green-600" /> General Notes
                </h4>
