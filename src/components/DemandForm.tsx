...
import { PlusCircle, Calendar, UploadCloud, FileText, Loader2, X, Image as ImageIcon, FileBox } from "lucide-react";
              ...
              <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" name="title" required className="w-full mt-1.5 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-900 font-medium" placeholder="e.g. Urea 46% Prilled" />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Quantity</label>
                      <input type="number" name="quantity" required className="w-full mt-1.5 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-900 font-medium" />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Target Price ($)</label>
                      <input type="number" step="0.01" name="targetPrice" required className="w-full mt-1.5 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-900 font-medium" />
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="relative mt-1.5">
                      ...
                      <input type="text" name="timeline" required className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-slate-900 font-medium" placeholder="e.g. Q3 2026, CIF Dubai" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Specifications / Notes</label>
                ...
                <div>
                  ...
                  <div className={`border-2 border-dashed rounded-2xl p-2 transition-colors relative ${pdfFile ? 'border-blue-200 bg-blue-50/50' : 'border-slate-200 bg-slate-50 hover:bg-slate-100/50'}`}>
                    {pdfFile ? (
                      <div className="flex items-center justify-between p-2 bg-white rounded-xl border border-blue-100 shadow-sm relative z-20">
                      ...
                      <>
                        ...
                        <div className="flex items-center justify-center gap-3 p-4 pointer-events-none text-slate-500">
                          <UploadCloud size={20} />
                          <span className="text-sm font-semibold">Attach RFQ Document</span>
                        </div>
                      </>
                    )}
                  </div>
