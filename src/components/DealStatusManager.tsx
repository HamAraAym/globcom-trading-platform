...
export default function DealStatusManager({ itemId, currentStatus, type, chatId, canEdit }: Props) {
  ...
  if (!canEdit) {
    return (
      <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold tracking-widest border ${activeStatus.color}`}>
        {activeStatus.label}
      </span>
    );
  }
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        disabled={isUpdating}
        className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-[10px] font-bold tracking-widest border shadow-sm transition-all hover:opacity-80 ${activeStatus.color} ${isUpdating ? "opacity-50 cursor-not-allowed" : ""}`}
      >
        {isUpdating ? <Loader2 size={12} className="animate-spin" /> : activeStatus.label}
        <ChevronDown size={12} />
      </button>

      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
          <div className="bg-slate-50 px-3 py-2 border-b border-slate-100 text-[9px] font-black text-slate-400 uppercase tracking-widest">
            Update Phase
          </div>
