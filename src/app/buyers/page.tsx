...
import { 
  Users, Building, Mail, Phone, ShieldCheck, CheckCircle2, AlertCircle, 
  Briefcase, MapPin, ShieldAlert, Clock, ExternalLink, User, Globe, 
  UserMinus, Save
} from "lucide-react";
...
export default async function BuyersPage() {
  // Fetch clients and reps simultaneously for faster loading
  const [clients, reps] = await Promise.all([
    ...
    prisma.user.findMany({
      // ⚡ EXPANDED: Allow assigning both Trading Reps AND dedicated Buyer Reps
      where: { role: { in: ["TRADING_REP", "BUYER_REP"] } }, 
  ...
  return (
    <div className="flex flex-col flex-1 p-4 md:p-6 lg:p-10 font-sans min-h-full bg-slate-50">
      ...
      <div className="max-w-[1600px] mx-auto w-full mb-8 shrink-0 flex flex-col lg:flex-row lg:items-start justify-between gap-6">
        ...
        <div className="flex flex-col items-end gap-3 w-full lg:w-auto">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <div className="hidden lg:flex items-center gap-2 bg-blue-50 text-blue-800 px-4 py-2.5 rounded-xl border border-blue-200 font-bold text-sm shadow-sm">
              <ShieldCheck size={18} className="text-blue-600" /> KYC Controlled
            </div>
      ...
      <div className="max-w-[1600px] mx-auto w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 mb-8 shrink-0">
        ...
        <div className="bg-white p-5 md:p-6 rounded-[20px] border border-rose-200 shadow-[0_4px_20px_-4px_rgba(244,63,94,0.1)] flex items-center gap-4 sm:col-span-2 md:col-span-1">
      ...
      <div className="max-w-[1600px] mx-auto w-full flex-1 bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
        ...
        <div className="bg-slate-900 px-6 md:px-8 py-5 md:py-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 shrink-0">
          ...
          <div className="bg-slate-800 border border-slate-700 text-slate-300 text-[11px] font-bold px-4 py-2 rounded-full shadow-sm w-fit uppercase tracking-wider">
              ...
              {clients.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-16 text-center text-slate-400 bg-slate-50/50">
                    <Briefcase size={48} className="mx-auto mb-4 opacity-20 text-slate-500" />
                    ...
                    <tr key={client.id} className="hover:bg-blue-50/30 transition-colors group">
                      ...
                      <td className="px-6 md:px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border shrink-0 ${client.type === 'CORPORATE' ? 'bg-blue-50 text-blue-700 border-blue-200 shadow-sm' : 'bg-slate-50 text-slate-700 border-slate-200 shadow-sm'}`}>
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1.5">
                               {client.company ? <User size={12} className="shrink-0 text-slate-400" /> : <MapPin size={12} className="shrink-0 text-slate-400" />}
                      ...
                      <td className="px-6 md:px-8 py-5">
                        ...
                        {client.kycStatus === "PENDING" && (
                          <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-amber-700 bg-amber-50 px-3 py-1.5 rounded-lg border border-amber-200">
                            <Clock size={14} /> Pending Docs
                          </span>
                        )}
                        {client.kycStatus === "REJECTED" && (
                          <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-rose-700 bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-200">
                      ...
                      <td className="px-6 md:px-8 py-5">
                        <form action={assignRep.bind(null, client.id)} className="flex items-center gap-2">
                          <select 
                            ...
                            className={`w-full max-w-[180px] text-[13px] rounded-lg p-2.5 font-bold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500/20 appearance-none cursor-pointer hover:shadow-sm ${
                          ...
                          <button 
                            ...
                            className={`p-2.5 rounded-lg transition-all flex items-center justify-center shrink-0 border shadow-sm ${
                              isAssigned 
                                ? 'bg-white border-slate-200 text-slate-400 hover:text-green-600 hover:bg-green-50 hover:border-green-200' 
                                : 'bg-rose-600 border-rose-600 text-white hover:bg-rose-700 hover:border-rose-700'
                            }`}
                          >
                            {isAssigned ? <CheckCircle2 size={16} /> : <Save size={16} />}
                          </button>
                        </form>
                      </td>
